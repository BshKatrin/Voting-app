...
class Graph(QWidget):
    ...
    def initUI(self):
        ...
        layout.addWidget(self.electorsTextBox)

        self.candidatesTextBox = QLineEdit(self)
        self.candidatesTextBox.setPlaceholderText("Enter number of random candidates")
        layout.addWidget(self.candidatesTextBox)
        
        self.electorsTextBox.returnPressed.connect(self.handleElectorsInput)  #   connecte les zones de text avec la touche enter et appelle la fonction après enter
    ...
    def drawGrid(self, painter):
        ...
        painter.setPen(pen)  #   tout les dessins appelant QPainter donc tout ce qui est déssiné ensuite utilisera le pinceau défini
    ...
    def drawPoints(self, painter):
        offset = QPoint(self.width() / 2, self.height() / 2)  #   position du milieu (0,0)
        ...
        for name,pos in self.candidates:
            painter.setPen(pen2)  #   reconfiguration du pinceau après une itération de la boucle
            widget_pos = QPoint(pos.x() + offset.x(), offset.y() - pos.y())
            painter.drawPoint(widget_pos)
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:  #   cas du clique gauche
            ...
            cartesian_pos = QPoint(event.pos().x() - offset.x(), offset.y() - event.pos().y())
            ...
            self.electors_positions.append(((cartesian_pos.x() / (self.width() // 2)), (cartesian_pos.y() / (self.height() // 2))))
        ...
        if event.button() == Qt.RightButton:  #   cas du clique droit
            self.createTextBox(event.pos())  #   appelle de la création de la zone de texte



    def createTextBox(self, position):
        self.text_box = QLineEdit(self)  #   création de la zone de texte
        self.text_box.move(position)  #   placement de la zone de texte à la position clique
        self.text_box.returnPressed.connect(lambda: self.storeName(position))  #   appelle de la fonction de création et de stockage du nom connecté avec la touche enter
    ...
    def storeName(self, position):
        ...
        print(f"Texte stocké : {name}")  #   affiche le texte stocké pour les tests (à enlever)
        ...
        self.candidates_positions.append((name,((cartesian_pos.x() / (self.width() // 2)), (cartesian_pos.y() / (self.height() // 2)))))
        self.update()
    ...
    def randomCandidatesCreation(self, n):
        for _ in range(n):
            ...
            y = random.randint(-self.height() // 2, self.height() // 2)
            cartesian_pos = QPoint(x, y)
            random_name = ''.join(random.choices(string.ascii_letters, k=10))  #   création de nom random
            
            self.candidates.append((random_name,cartesian_pos))
            
            self.candidates_positions.append((random_name,((cartesian_pos.x() / (self.width() // 2)), (cartesian_pos.y() / (self.height() // 2)))))
        self.update()
    ...
    def showPositions(self):
        ...
        positions_text = '\n'.join([f'Position: {pos[0]}, {pos[1]}' for pos in self.electors_positions])
        print(positions_text)

        positions_text2 = '\n'.join([f'Position: {pos[0]}, {pos[1]}' for pos in self.candidates_positions])

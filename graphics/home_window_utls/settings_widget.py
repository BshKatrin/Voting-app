...
class SettingsWidget(QWidget):
    """Une classe qui représente un widget avec des réglages d'une élection."""
    sig_saved = Signal()
    ...
    def setPollVotingRule(self, voting_rule_ui: str) -> None:
        """Définir pour quelle règle du vote les sondages seront effectuées."""
    ...
    def toggleLiquidDemocracy(self, state: int) -> None:
        """Activer ou désactiver une démocratie liquide
        (i.e. si les électeurs pourront ou pas faire des délégations)"""
        self.election.liquid_democracy_activated = bool(state)

    @Slot(int)
    def toggleTieBreaker(self, state: int) -> None:
        """Un slot permettant d'activer ou désactiver un tie-break selon les duels."""
        self.election.tie_breaker_activated = bool(state)

    @Slot()
    def saveSettings(self) -> None:
        """Emettre le signal `sig_saved`"""
        self.sig_saved.emit()
